...
public class ServiceMqtt extends Service{
    ...
    ArrayList<String> arrayListSubscribeTopic = new ArrayList<String>();//记录ClientID
    @Override
    public void onCreate() {
        //在使用HandlerThread的getLooper()方法之前，必须先调用该类的start()，同时开启一个新线程;

        mDataBaseDeviceMessage = new DataBaseDeviceMessage(getApplicationContext(), "DevicesMsgDatabase.db", 1);
    ...
    private void InitMqttConnect()
    {
        try
        {
            ...
            Cursor cursor = mDataBaseDeviceMessage.query(DataBaseDeviceMessage.DeviceClientID, null, null, null, null, null, null, null);
            while (cursor.moveToNext())
            {
                String mString = cursor.getString(cursor.getColumnIndex(DataBaseDeviceMessage.DeviceClientID));
                arrayListSubscribeTopic.add(mString);
            }
            cursor.close();
            mDataBaseDeviceMessage.close();
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1)
            {
                try
                {
                    ...
                    for (String SubscribeTopic :arrayListSubscribeTopic)
                    {
                        mqttClient.subscribe("user/"+SubscribeTopic,0);
                    }
            ...
            try
            {
                ...
                if (msgString != null)
                {
                    ...
                    if (tempString[0].equals("ResetMqtt")) {
                        ...
                        MqttMessage msgMessage = new MqttMessage(tempString[2].getBytes());
                        try {
                            mqttClient.publish(tempString[1],msgMessage);
...
//                            Log.e("err",e.toString());
                    ...
                    else if (tempString[0].equals("SubscribeTopic")){//订阅
                        mqttClient.subscribe("user/"+tempString[1],0);
                    }
                    else if (tempString[0].equals("UnSubscribUneTopic")){//取消订阅
                        mqttClient.unsubscribe("user/"+tempString[1]);
                    }
                }
            }
            catch (Exception e) {
